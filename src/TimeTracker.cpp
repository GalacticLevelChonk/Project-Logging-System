...
void AddInfo(){
    ...
    std::cout << "\nInput your intentions: ";
    getline(std::cin, intention);

    std::cout << "\n\nWould you like to add tasks?(type yes if you want to):  ";
    ...
    if(addTasks == "yes" || addTasks == "Yes"){
        ...
        while(true){
            ...
            if(task == "delete" || task == "Delete"){
                ...
                std::cout << "\nDelete at what index(starting at 1 and to clear ALL type -314)";
                std::cin >> index;
...
void DisplayHome(){
    ...
    std::cout << "Add: Adds information to the current session\n";
    std::cout << "Pause: Pauses the current session for a later time, stores data even after program shut off\n";
    std::cout << "Continue: Continues a previously paused session";
}
